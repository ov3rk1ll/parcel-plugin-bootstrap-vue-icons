  ...
  async addAsset(asset) {
    if (
      ...
      const packageOptions =
        packageContent && packageContent.bootstrapVueIcons
          ? packageContent.bootstrapVueIcons
          : {};

      const options = Object.assign({ icons: [] }, packageOptions);
      if (options.icons.length > 0) {
        options.icons = options.icons.map((x) => this.snakeToPascalCase(x));
